        ...
        stage('Update Helm Chart') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-token', usernameVariable: 'GH_USER', passwordVariable: 'GH_TOKEN')]) {
                    sh """
                        git clone -b ${CD_BRANCH} https://${GH_USER}:${GH_TOKEN}@github.com/BunathPheng/CD-banking-service.git cd-repo
                        cd cd-repo
                        ...
                        git push https://${GH_USER}:${GH_TOKEN}@github.com/BunathPheng/CD-banking-service.git ${CD_BRANCH}
                    """
                }
            }
        }
